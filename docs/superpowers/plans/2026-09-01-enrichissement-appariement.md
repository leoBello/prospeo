...
export function createGoogleMapsSource(options: GoogleMapsOptions): MapsSource {
    ...
    async search(query: string): Promise<MapsCandidate[]> {
      ...
      try {
        ...
        for (const card of cards.slice(0, settings.maxCandidates)) {
          const href = await card.locator(SELECTORS.cardLink).first().getAttribute('href');
        ...
        for (const raw of raws) {
          ...
          const merged = detail ?? raw;
          const candidate = toMapsCandidate({ ...merged, placeUrl: raw.placeUrl });
  ...
  async function textOf(scope: Page | Locator, selector: string): Promise<string | null> {
    return scope.locator(selector).first().textContent().catch(() => null);
  }

  async function attrOf(scope: Page | Locator, selector: string, attribute: string): Promise<string | null> {
    return scope.locator(selector).first().getAttribute(attribute).catch(() => null);
  }
...
- [ ] **Étape 3 : vérification manuelle sur une recherche réelle**

Les sélecteurs ci-dessus sont une hypothèse de départ : Google renomme
régulièrement ses classes. Cette étape n'est pas facultative.

```bash
pnpm --filter @prospeo/collector exec tsx -e "
import { createGoogleMapsSource } from './src/sources/google-maps.js';
const source = createGoogleMapsSource({ userDataDir: '.playwright-profile', headless: false });
console.log(await source.search('plombier Nantes'));
await source.close();
"
```

Attendu : entre 1 et 5 fiches, chacune portant au minimum un `name` et un
`mapsUrl`. **Si un champ ressort systématiquement à `null`, corriger le
sélecteur correspondant dans `SELECTORS` avant de poursuivre** — un champ
muet produirait des `not_found` massifs qu'aucun test ne signalerait.
